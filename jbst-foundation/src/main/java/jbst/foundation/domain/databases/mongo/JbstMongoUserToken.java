...
@ToString
// Mongodb
@Document(collection = JbstMongoUserToken.MONGO_TABLE_NAME)
public class JbstMongoUserToken {
    ...
    public JbstMongoUserToken(JbstRequestUserToken request) {
        this(
                request.email(),
    ...
    public JbstMongoUserToken(JbstUserToken token) {
        this(
                token.email(),
                token.value(),
                token.type(),
    ...
    public static JbstMongoUserToken random(
            Username username,
            JbstUserTokenType type,
            long expiryTimestamp,
            boolean used
    ) {
        return new JbstMongoUserToken(
    ...
    public static List<JbstMongoUserToken> dummies1() {
        var token1 = JbstMongoUserToken.random(
                Username.of("username1"),
                JbstUserTokenType.EMAIL_CONFIRMATION,
                getFutureTimestamp(Duration.ofDays(1L)).value(),
                false
        );
        var token2 = JbstMongoUserToken.random(
                Username.of("username2"),
                JbstUserTokenType.PASSWORD_RESET,
                getFutureTimestamp(Duration.ofDays(1L)).value(),
                false
        );
        var token3 = JbstMongoUserToken.random(
                Username.of("username3"),
                JbstUserTokenType.EMAIL_CONFIRMATION,
                getPastTimestamp(Duration.ofDays(1L)).value(),
                false
        );
        var token4 = JbstMongoUserToken.random(
                Username.of("username4"),
                JbstUserTokenType.PASSWORD_RESET,
                getPastTimestamp(Duration.ofDays(1L)).value(),
                false
        );
        var token5 = JbstMongoUserToken.random(
                Username.of("username5"),
                JbstUserTokenType.EMAIL_CONFIRMATION,
                getPastTimestamp(Duration.ofDays(1L)).value(),
                true
        );
        var token6 = JbstMongoUserToken.random(
