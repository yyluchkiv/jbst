...
// JPA
@Entity
@Table(name = JbstPostgresUserToken.PG_TABLE_NAME)
public class JbstPostgresUserToken extends JbstPostgresAbstractPersistable0 {
    ...
    public JbstPostgresUserToken(JbstRequestUserToken request) {
        this(
                request.email(),
    ...
    public JbstPostgresUserToken(JbstUserToken token) {
        this(
                token.email(),
                token.value(),
                token.type(),
    ...
    public static JbstPostgresUserToken random(
            Username username,
            JbstUserTokenType type,
            long expiryTimestamp,
            boolean used
    ) {
        return new JbstPostgresUserToken(
    ...
    public static List<JbstPostgresUserToken> dummies1() {
        var token1 = JbstPostgresUserToken.random(
                Username.of("username1"),
                JbstUserTokenType.EMAIL_CONFIRMATION,
                getFutureTimestamp(Duration.ofDays(1L)).value(),
                false
        );
        var token2 = JbstPostgresUserToken.random(
                Username.of("username2"),
                JbstUserTokenType.PASSWORD_RESET,
                getFutureTimestamp(Duration.ofDays(1L)).value(),
                false
        );
        var token3 = JbstPostgresUserToken.random(
                Username.of("username3"),
                JbstUserTokenType.EMAIL_CONFIRMATION,
                getPastTimestamp(Duration.ofDays(1L)).value(),
                false
        );
        var token4 = JbstPostgresUserToken.random(
                Username.of("username4"),
                JbstUserTokenType.PASSWORD_RESET,
                getPastTimestamp(Duration.ofDays(1L)).value(),
                false
        );
        var token5 = JbstPostgresUserToken.random(
                Username.of("username5"),
                JbstUserTokenType.EMAIL_CONFIRMATION,
                getPastTimestamp(Duration.ofDays(1L)).value(),
                true
        );
        var token6 = JbstPostgresUserToken.random(
