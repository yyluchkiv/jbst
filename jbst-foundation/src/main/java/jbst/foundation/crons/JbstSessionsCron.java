...
public class JbstSessionsCron extends AbstractBaseCron {
    ...
    @Scheduled(
            cron = "${jbst.security.session-configs.clean-sessions-by-expired-refresh-tokens-cron.expression}",
            zone = "${jbst.security.session-configs.clean-sessions-by-expired-refresh-tokens-cron.zone-id}"
    )
    public void cleanByExpiredRefreshTokens() {
        this.executeCron(
                this.jbstProperties.getSecurity().getSessionConfigs().getCleanSessionsByExpiredRefreshTokensCron().isEnabled(),
                () -> {
    ...
    @Scheduled(
            cron = "${jbst.security.session-configs.enable-sessions-metadata-renew-cron.expression}",
            zone = "${jbst.security.session-configs.enable-sessions-metadata-renew-cron.zone-id}"
    )
    public void enableSessionsMetadataRenew() {
        this.executeCron(
                this.jbstProperties.getSecurity().getSessionConfigs().getEnableSessionsMetadataRenewCron().isEnabled(),
